...
// In-memory fallback if localStorage is unavailable or blocked in iframe
...
function getLocalData<T>(key: string, seed: T): T {
  try {
    ...
    if (!raw) {
      safeStorage.setItem(key, JSON.stringify(seed));
      return seed;
    }
...
function setLocalData<T>(key: string, data: T): void {
  try {
    safeStorage.setItem(key, JSON.stringify(cleanPayload(data)));
  } catch (err) {
...
export const StorageService = {
  ...
  resetDemoData(): void {
    ...
    safeStorage.setItem(STORAGE_KEYS.CHALLENGES, JSON.stringify(SEED_CHALLENGES));
    safeStorage.setItem(STORAGE_KEYS.SOLUTIONS, JSON.stringify(SEED_SOLUTIONS));
    safeStorage.setItem(STORAGE_KEYS.PROJECTS, JSON.stringify(SEED_PROJECTS));
    safeStorage.setItem(STORAGE_KEYS.COMMENTS, JSON.stringify(SEED_COMMENTS));
    safeStorage.setItem(STORAGE_KEYS.NOTIFICATIONS, JSON.stringify(SEED_NOTIFICATIONS));
    safeStorage.setItem(STORAGE_KEYS.REPORTS, JSON.stringify(SEED_REPORTS));
  },
